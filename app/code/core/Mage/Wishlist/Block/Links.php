    ...
    protected function _createLabel($count)
    {
        if(!Mage::getSingleton('customer/session')->isLoggedIn()){
            ...
            if( Mage::getSingleton('core/session')->getWishListGuest() && Mage::getSingleton('core/session')->getWishListGuest() != '')
            {
                ...
                $count  = count($_productIds);
            }
            if ($count > 0) {
                return $this->__('My Wishlist ('."<span class='wishlist-guest'>".'%d items'."</span>".')',$count);
            } else {
                return $this->__('My Wishlist ('."<span class='wishlist-guest'></span>".')');
            }
        }
        else {
            if ($count >= 1) {
